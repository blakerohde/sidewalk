...
class ActivityAggregator:
    ...
    def run(self):
        ...
        for (key, activity_processor) in self.active_activity_processor_pairs.items():  
            ...
            loggers._debug('callable: ' + attr)
            loggers._debug('location: %s' % path)
            ...
            for i in reversed(range(1, len(path)+1)):
                try:
                    loggers._debug('import \'' + '.'.join(path[:i]) + '\'?')
                    module = self.import_module('.'.join(path[:i]))
                    loggers._debug(module)
                except ImportError:
                    loggers._debug('no.')
                    loggers._debug(traceback.format_exc())
                    continue
                
                if module is not None:
                    while True:
                        try:
                            loggers._debug('sub-import \'' + '.'.join(path[1:i+1]) + '\'?')
                    ...
                    if module is not None:
                        try:
                            loggers._debug('get attr \'' + attr + '\' in ' + str(module) + '?')
